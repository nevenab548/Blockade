...
class Main:
    ...
    def is_move_valid(self, move):
        reg = re.compile('\[[XO] [12]] \[[A-Fa-f0-9]+ \d] \[[BG] \d \d]')
    ...
    def check_if_goal_is_not_blocked(self, player, wall, wall_color):
        ...
        is_valid = False
        if wall_color == "B":
            self.table.matrix[int(wall[0]) - 1][int(wall[1]) - 1].set_lower_wall(1)
            self.table.matrix[int(wall[0]) - 1][int(wall[1])].set_lower_wall(1)
            self.table.matrix[int(wall[0])][int(wall[1]) - 1].set_upper_wall(1)
            self.table.matrix[int(wall[0])][int(wall[1])].set_upper_wall(1)
        else:
            self.table.matrix[int(wall[0]) - 1][int(wall[1]) - 1].set_right_wall(1)
            self.table.matrix[int(wall[0])][int(wall[1]) - 1].set_right_wall(1)
            self.table.matrix[int(wall[0]) - 1][int(wall[1])].set_left_wall(1)
            self.table.matrix[int(wall[0])][int(wall[1])].set_left_wall(1)
        self.graph = Graph(self.table)
        if player.player_color == "X":
            if (self.graph.is_there_path(player.pawn_one_position, [7, 3]) is True or self.graph.is_there_path(
                    player.pawn_two_position, [7, 3]) is True) and \
                    (self.graph.is_there_path(player.pawn_one_position, [7, 10]) is True or self.graph.is_there_path(
                        player.pawn_two_position, [7, 10]) is True):
                is_valid = True
        else:
            if (self.graph.is_there_path(player.pawn_one_position, [3, 3]) is True or self.graph.is_there_path(
                    player.pawn_two_position, [3, 3]) is True) and \
                    (self.graph.is_there_path(player.pawn_one_position, [3, 10]) is True or self.graph.is_there_path(
                        player.pawn_two_position, [3, 10]) is True):
                is_valid = True
        # ponistavanje zida
        if wall_color == "B":
            self.table.matrix[int(wall[0]) - 1][int(wall[1]) - 1].set_lower_wall(0)
            self.table.matrix[int(wall[0]) - 1][int(wall[1])].set_lower_wall(0)
            self.table.matrix[int(wall[0])][int(wall[1]) - 1].set_upper_wall(0)
            self.table.matrix[int(wall[0])][int(wall[1])].set_upper_wall(0)
        else:
            self.table.matrix[int(wall[0]) - 1][int(wall[1]) - 1].set_right_wall(0)
            self.table.matrix[int(wall[0])][int(wall[1]) - 1].set_right_wall(0)
            self.table.matrix[int(wall[0]) - 1][int(wall[1])].set_left_wall(0)
            self.table.matrix[int(wall[0])][int(wall[1])].set_left_wall(0)
        self.graph = Graph(self.table)
        return is_valid
    ...
    def main(self):
        ...
        while self.is_it_end():
            self.show_table()
            self.make_move()
            # posle svakog poteza pravi se novi graf
        # ocictiti konzolu
